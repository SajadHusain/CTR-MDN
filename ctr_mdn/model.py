...
class CTRForwardMDN(nn.Module):
    """MLP trunk + mixture density head with full covariances.

    Each component is parameterised by an upper-triangular factor U of its precision
    matrix (Lambda = U^T U). The diagonal of U goes through a softplus so that the
    covariance is always positive definite.
    """
