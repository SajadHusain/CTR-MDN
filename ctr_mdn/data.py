"""Dataset loading, joint encoding and sequence splits."""
import os
...
# tube lengths [mm] used in the translation mapping M_B (tube 1 = innermost)
...
class CTRFKDataset(Dataset):
    ...
    samples are replaced by the first sample of that sequence.

    Sequences: by default the rows are split into n_seq contiguous blocks of equal length
    (rows that do not fill a block are dropped). With seq_len given, every run of seq_len
    consecutive rows is one sequence and the last one may be shorter.
    """

    def __init__(self, csv_path, memory_len=3, n_seq=N_SEQ, seq_len=None):
